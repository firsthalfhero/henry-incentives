// Shared server-side logic for every child's earning chart. Deliberately has
// NO npm dependencies of its own (no `@netlify/blobs` import here) — each
// child's own netlify/functions/state.mjs imports `getStore` itself, from
// inside its own Netlify site's base directory, so Netlify's ordinary
// `npm install` (scoped to that base directory) is all dependency resolution
// this file ever needs. This file is imported across that base-directory
// boundary via a plain relative path, which Netlify's Functions bundler
// resolves by filesystem path regardless of "base directory" (confirmed
// against a real deploy before this design was adopted).
const DOC_KEY = "state";
...
// in sync manually (same reasoning as before: the server independently
...
// Builds this child's request handler. `activitiesData` is that child's own
// activities.json (imported by their own state.mjs); `storeName` isolates
// their Blobs store; `getStore` is passed in rather than imported here so
// this file stays dependency-free (see comment at top).
export function makeHandler({ activitiesData, storeName, getStore }) {
  const SPEND_CAP = (activitiesData && typeof activitiesData.spendCap === "number")
    ? activitiesData.spendCap
    : 40; // fallback matches the value hardcoded before spendCap moved into activities.json
